<div class="row">
  <div class="col-4">
    ...
    @else
        <i class="fa-solid fa-circle-user text-secondary b-block text-center icon-lg"></i>
    @endif
  </div>
  <div class="col-8">
    <div class="row mb-3">
        ...
        @else
            @if ($user->isFollowed())
              <form action="{{ route('follow.destroy', $user->id) }}" method="post" class="d-inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-primary btn-sm fw-bold">Following</button>
              </form>
            @else
